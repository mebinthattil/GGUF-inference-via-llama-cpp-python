...
class GGUFInference:
    ...
    def __init__(self, model_path: str, max_context_tokens: int = 1500, generation_mode: int = 1, n_threads: int = 1, verbose: bool = False):
        '''ARGS:
        max_context_tokens: For the model used the actual max context window is 2048, but reducing cause we use an approximation while calculating the token count (*1.3)
        ...
        self.conversation_history : List[ Dict["student": str, "teacher": str] ] = []
        ...
        self.generation_params = {
            ...
            "stop": ["Student:", "\nStudent:"]  # Stop sequences
        }
        
        print(f"Model loaded successfully!")
        print(f"Temperature: {self.generation_settings['temperature']}")
        print(f"Max tokens per response: {self.generation_settings['max_tokens']}")
    ...
    def _truncate_history_if_needed(self, new_student_input: str) -> str:
        ...
        -> else, remove the oldest conversation entry then check again if limit hit...
        -> until you reach a part where along with the current new question we are below limit - once that is found - return that
        """
        history_str = self._format_conversation_history()
        potential_instruction = f"{history_str}Student: {new_student_input}\nTeacher:"
        
        token_count = len(potential_instruction.split()) * 1.3 # this is an approximation
        ...
        for i in range((len(self.conversation_history)- 1), -1, -1):
            ...
            for entry in temp_history:
                temp_str += f"Student: {entry['student']}\nTeacher: {entry['teacher']}\n" # bringing it to the required format
            #temp_str now looks like: Student: question\nTeacher: answer\nStudent: question2\nTeacher: answer2\n ... till conversation_history[i:]


            test_instruction = f"{temp_str}Student: {new_student_input}\nTeacher:" # to check if after the new student question will we cross context limit
